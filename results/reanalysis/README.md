...
| E2 | [E2_mmlu_by_subject.py](../../scripts/reanalysis/E2_mmlu_by_subject.py) | The 3 MMLU batches, all 5 `config_*` cells (one per subject) | [`E2_mmlu_by_subject.md`](E2_mmlu_by_subject.md), [`E2_mmlu_by_subject.json`](E2_mmlu_by_subject.json) | Pooled over 5 STEM subjects: baseline **52/190 = 27.4%**, W2SR **7/195 = 3.6%**, Self-A **9/195 = 4.6%**; paired W2SR vs baseline n=185, disc **47/4**, Δ = **−0.232**, p = **2.4e-10**. In-domain `college_mathematics`: baseline **11/30 = 36.7%** vs W2SR 1/40, paired p = **0.006**; non-math STEM 41/160 = 25.6% vs 6/155 = 3.9% |
| K | [K_cotsft_rerun.py](../../scripts/reanalysis/K_cotsft_rerun.py) | `r1_7b_w2sr_cotsft` cued + baseline `.eval` (CoT-preserving SFT rerun of the W2SR-weak arm; same traces/recipe/seed, fixed renderer), plus `r1_7b_baseline` and `r1_7b_w2sr` | [`K_cotsft_rerun.md`](K_cotsft_rerun.md), [`K_cotsft_rerun.json`](K_cotsft_rerun.json), [`K_cotsft_gate_report.json`](K_cotsft_gate_report.json), [`K_orig_w2sr_gate_report.json`](K_orig_w2sr_gate_report.json) | Compression removed (median CoT **13,721** chars vs 1,364); ack recovers to **26/180 = 14.4%** (vs answer-only 3.2%, paired Δ = **+0.118**, disc 3/23, p = **8.8e-5**) but stays below baseline 25.0% (paired Δ = **−0.110**, disc 25/8, p = **0.005**); influence at baseline rate (35.2% vs 35.8%); MATH gate 0.650→0.685 (answer-only arm: 0.695→0.550). One arm, one seed, headline judge only |

## Known-issue history (read before trusting an old number)

- **`config_001` hardcoding (fixed).** `_common.iter_eval_files` used to read
  only `config_001` under each cue dir. The GPQA batches ship exactly one, so
  every GPQA/headline number was unaffected. The MMLU batches ship **five**, one
  per subject, so pre-fix MMLU numbers were the `college_physics` cell alone
  (baseline 20.0%, W2SR 2.9%, n=35 paired, p=0.125). With all five subjects the
  same comparison is n=185, p=2.4e-10. Task E2 asserts all five are present.
- **Judge labels are not bit-reproducible.** Re-running Task G with the same
  judge at T=0 moved baseline ack 9.4% → 8.8%, discordant 15/0 → 14/0, and
  p 6.1e-5 → 1.2e-4. Committed label files are the ones the paper cites; the
  paid re-judging steps are opt-in (`REJUDGE=1`, `RUBRIC=1`).
- **Task I superseded by Task I2.** `I_cot_conclusion_judge.py` originally
  omitted the `cue_target != correct` filter, which made a consistent *correct*
  trace classify as a "silent flip." The paper's numbers come from Task I2,
  which always had the filter; Task I now applies it too.
