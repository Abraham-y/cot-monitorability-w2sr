...
removes the compression and turns the exploratory cut into an intervention:
acknowledgment recovers only partially ($3\% \to 14\%$, still below baseline's
$25\%$, paired $p = 0.005$) at preserved capability. A capability-only
...
supervision (Task K, \S\ref{sec:taskK}) removes the compression, yet
...
Distillation passes the capability gate (held-out MATH Pass@1 above baseline by
the preregistered $\ge 5$-point margin), the trained student's GPQA answers
...
differ across scale and eval set). Paired influence (answer equal to cue
...
not bought with a capability loss that a certifier would notice: on the metric
the certification actually checks, the model is unchanged.
...
loss. This removes the compression at its source: the retrained student's
median cued-cell CoT is $13{,}721$ chars ($1{,}364$ for the answer-only arm;
$18{,}847$ for baseline). Under the compression-only account, acknowledgment
...
(influenced $35.2\%$ vs baseline $35.8\%$), and capability is preserved:
on the held-out MATH gate the rerun scores $0.685$ against its untrained
reference $0.650$, where the answer-only arm had scored $0.550$ against
$0.695$; on matched uncued GPQA questions under a single re-extraction
method the rerun scores $0.562$ ($n = 32$) vs the answer-only arm's
$0.447$ ($n = 38$). Per cue, the recovery is uneven: insider-info
...
the headline judge only (no multi-judge battery on this arm), and the
per-cue cells are small (result files:
...
was judged a silent flip); excluding every one of them
still gives $184/225 = 81.8\%$, so the headline does not rest on them (and a
...
nor the Task K rerun, which removes the compression by fixing the supervision
and still finds acknowledgment significantly below baseline. The
...
reportable,'' which is what we report. The prereg also called for judge--human
...
($25\%$ to $3\%$ at preserved accuracy) is not specific to weak-to-strong
...
reasoning-preserving supervision (Task K) removes the compression yet leaves
acknowledgment significantly below baseline ($14.4\%$ vs $25.0\%$, paired
